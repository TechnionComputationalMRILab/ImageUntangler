...
def build(folder, case_name):
    ...
    #
    # dicompiler_db = Database(folder)
    # build_sequence_tables(dicompiler_db, case_id, case_name)
    # build_metadata_table(dicompiler_db, case_id, case_name)

    # build_slice_location_table(dicompiler_db, case_id)


# def build_slice_location_table(dicompiler_db, case_id):
#     db_file = CFG.get_db()
...
#
#
# def build_metadata_table(dicompiler_db, case_id, case_name):
...
def build_sequence_tables(dicompiler_db, case_id, case_name):
    """
    ...
    with con:
        if len(dicompiler_db.cases) > 1:
            raise NotImplementedError
        else:
            case = dicompiler_db.cases[0]
            for seq_id, seq in enumerate(case.sequences):
                seq_name = seq.metadata[DICOMTag.SeriesDescription]

                con.execute('insert into sequences (case_id, name, seq_id, orientation) values (?, ?, ?, ?)',
                                                           (case_id, seq_name, seq_id + 1, "X"))
    ...
    #
    # single_sequence_file_list = []
    # single_sequence_list = []
    # single_sequence_flag = False # if a sequence with one file is found
    # single_sequence_orientation = "UNKNOWN"
    #
    # last_seq_id = 0
    ...
    #         print(f"Error found in getting image orientation: {e}")
